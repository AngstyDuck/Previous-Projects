...
def linear_regression(bunchobject, x_index, y_index, size, seed):
    ...
    coeff_x1 = np.array([[output_polyfit[0]]])
...
def multiple_linear_regression(bunchobject, x_index, y_index, order, size, seed):
    ...
    """
    #----------------------------Didn't work-------------------------
    output_polyfit = np.polyfit(np.reshape(data_test,(1,-1))[0], np.reshape(y_pred,(1,-1))[0],order+1)
    coeff_x0 = np.array([output_polyfit[-1]])
    ...
    #extra functions to transform x_train and x_test
    shape_data_train = data_train.shape
    shape_data_test = data_test.shape

    #return data_train.reshape[:0], target_train, data_test[:0], y_pred, output_result
    return output_result

bunchobject = datasets.load_breast_cancer()
print(multiple_linear_regression(bunchobject,0,3,4,0.4,2752))
...
#
#
# print(knn_classifier_full(bunchobject, range(20), 0.4, 2752))
